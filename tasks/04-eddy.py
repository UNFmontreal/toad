...
class Eddy(GenericTask):
    ...
    def implement(self):
        ...
        #QA
        translationsPng = self.buildName('translations', None, 'png')
        rotationPng = self.buildName('rotations', None, 'png')
        self.plotMovement(eddyParameterFiles, translationsPng, rotationPng)
        
        bVecsPng = self.buildName('vectors', None, 'png')
        self.plotvectors(bVecs, bVecsPng)
    ...
    def qaSupplier(self):
        ...
        """
        eddyGif = self.getImage(self.workingDir, "dwi", 'eddy', ext='png')
        translationsPng = self.getImage(self.workingDir, 'translations', ext='png')
        rotationsPng = self.getImage(self.workingDir, 'rotations', ext='png')
        bvecsPng = self.getImage(self.workingDir, 'vectors', ext='png')
        
        images = [(eddyGif,'DWI eddy'),
                  (translationsPng,'Translation correction by eddy'),
                  (rotationsPng,'Rotation correction by eddy'),
                  (bvecsPng,'Gradients vectors on the unitary sphere. Red : raw bvec | Blue : opposite bvec | Black + : movement corrected bvec')]
        return images
