...
class TractographyMrtrix(GenericTask):
    ...
    def isDirty(self):
        ...
        if self.__nbDirections <= 45:
            if 'deterministic' in self.get('algorithm'):
                images.extend(Images((self.getImage('dwi', 'tensor_det', 'trk'), "deterministic tensor connectome matrix from a streamlines")))

            if 'probabilistic' in self.get('algorithm'):
                images.extend(Images((self.getImage('dwi', 'tensor_prob', 'trk'), "probabilistic tensor connectome matrix from a streamlines")))

        else:
            if 'hardi' in self.get('algorithm'):
                images.append(Images((self.getImage('dwi', 'hardi_prob', 'trk'), "tckgen hardi probabilistic streamlines tractography")))

            if 'sift' in self.get('algorithm'):
                images.extend(Images((self.getImage('dwi', 'tcksift', 'trk'), 'tcksift')))
