...
class Eddy(GenericTask):
    ...
    def qaSupplier(self):
        ...
        if self.getImage(self.dependDir, 'b0_ap') and self.getImage(self.dependDir, 'b0_pa'):
            ...
            information = 'No distortion correction done during the pipeline'
        qaImages.setInformation(information)

        tags = [
            ('eddy', 'gif', 'DWI eddy corrected'),
            ('compare', 'gif', 'Before and after eddy corrections'),
            ('translations', 'png', 'Translation correction by eddy'),
            ('rotations', 'png', 'Rotation correction by eddy'),
            ('vectors', 'gif', 'Gradients vectors on the unitary sphere. Red : raw bvec | Blue : opposite bvec     | Black + : movement corrected bvec'),
            ]
