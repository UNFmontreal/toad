...
class TractFiltering(GenericTask):
    ...
    def implement(self):
        ...
        rawTractographies = self.workingDir + os.pathsep + 'raw'

        self.__linksTractographies(rawTractographies)

        configRemoveOutliers = self.get('configRemoveOutliers')
        self.__removeOutliers(configRemoveOutliers, rawTractographies, rawTractographies)

    def __linksTractographies(self, target):
        source = self.getTractQuerierImage(None, None, 'trk')

        if not os.path.exists(target):
            os.mkdir(target)

        for src in source:
            util.symlink(src, self.workingDir)

    def __removeOutliers(self, config, source, target):
